...
--statements xs ys = concat $ liftA2 (\x y -> [x ++ " is " ++ y, x ++ " is not " ++ y]) xs ys
statements xs ys = (\x y z -> x ++ y ++ z) <$> xs <*> [" is ", " is not "] <*> ys
...
--validateDiv a b = fmap (div a) (check (b /= 0) "Division by zero!" b)
...
validateAddress streetName streetNumber postCode =
  checkedStreetName *> checkedStreenNumber *> checkedPostCode
  where checkedStreetName =
          check (length streetName < 21) "Invalid street name" (Address streetName streetNumber postCode)
        checkedStreenNumber =
          check (all isDigit streetNumber) "Invalid street number" (Address streetName streetNumber postCode)
        checkedPostCode =
          check (all isDigit postCode && length postCode == 5) "Invalid postcode" (Address streetName streetNumber postCode)
...
boolOrInt s = validateBool s <|> validateInt s
  where
    validateBool s = case readMaybe s :: Maybe Bool of
                          Just b -> pure (Left b)
...
normalizePhone str = checkLength trimmed *> traverse checkCharacter trimmed
  where checkLength s = check (length s < 11) "Too long" s
        checkCharacter c = check (isDigit c) ("Invalid character: " ++ [c]) c
        trimmed = filter (/=' ') str
...
parseExpression str =
  ...
  then invalid ("Invalid expression: " ++ str)
  else checkedOperator *> checkedArg
    where
      checkedOperator =
        check (op == "+" || op == "-") ("Unknown operator: " ++ op) (Minus (Number 0) (Number 0))
      checkedArg =
        if op == "+"
        then liftA2 Plus (checkArg a1) (checkArg a2)
        else liftA2 Minus (checkArg a1) (checkArg a2)
      [a1, op, a2] = words str

checkArg :: String -> Validation Arg
checkArg arg = checkNumber <|> checkVariable
  where checkNumber = case readMaybe arg :: Maybe Int of
                           Just x -> pure (Number x)
                           Nothing -> invalid ("Invalid number: " ++ arg)
        checkVariable = check (length arg == 1 && isAlpha (head arg)) ("Invalid variable: " ++ arg) (Variable (head arg))
...
instance (Functor f, Functor g) => Functor (Both f g) where
  fmap q (Both x) = Both (fmap (fmap q) x)
