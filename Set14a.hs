...
greetText name = if T.length name < 16
                 then T.concat [T.pack "Hello, ", name, T.pack "!"]
                 else T.concat [T.pack "Hello, ", T.take 15 name, T.pack "...!"]
...
shout :: T.Text -> T.Text
shout = T.unwords . shout' . T.words

shout' :: [T.Text] -> [T.Text]
shout' [x] = [T.toUpper x]
shout' (x : y : rest) = (T.toUpper x) : y : shout' rest
shout' _ = []
...
longestRepeat text =
  case T.uncons text of
    Nothing -> 0
    Just (x, rest) -> max (countLetter x text) (longestRepeat rest)
...
pingpong b = BL.cycle $ BL.append d $ BL.reverse d
    where d = BL.fromStrict b
