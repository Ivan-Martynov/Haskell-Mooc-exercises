...
study (NthYear year) = if year < 7 then NthYear (year + 1) else Graduated
...
data UpDown = CountUp Int | CountDown Int
...
tick :: UpDown -> UpDown
tick (CountUp x) = CountUp (x + 1)
tick (CountDown x) = CountDown (x - 1)

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
toggle (CountUp x) = CountDown x
toggle (CountDown x) = CountUp x
...
rgb (Green) = [0, 1, 0]
rgb (Blue) = [0, 0, 1]
rgb (Mix a b) = zipWith (\x y -> (x + y) * 0.5) (rgb a) (rgb b)
...
toList (Empty) = []
toList (Pair x y tail) = (x, y) : (toList tail)

fromList :: [(k,v)] -> KeyVals k v
fromList = foldr (\(x, y) acc -> Pair x y acc) Empty
...
toNat z
    | z < 0 = Nothing
    | otherwise = Just (maybe Zero (\x -> PlusOne x) $ toNat (z - 1))
...
toBin 0 = O End
toBin 1 = I End
toBin n = if n `rem` 2 == 1 then I (toBin m) else O (toBin m) where m = n `div` 2
