...
inputInOutput input output = forAll (elements input) (\x -> (x `elem` map fst output) === True)
...
outputInInput input output =
  forAll (elements output) (\x -> (length $ filter (==(fst x)) input) === snd x)
...
--  conjoin [sumIsLength input output, inputInOutput input output, outputInInput input output]
