...
capitalize = unwords . map capitalizeFirst . words
    where capitalizeFirst (c : str) = toUpper c : str
...
--powers k max = map (k^) . takeWhile (\y -> k ^ y <= max) $ [0 .. max - 1]
...
--whileRight check x
--    | checkLeft' y = leftValue y
--    | checkRight' y = whileRight check . rightValue $ y
--        where y = check x

checkLeft' :: Either a b -> Bool
checkLeft' (Left _) = True
checkLeft' (Right _) = False

checkRight' :: Either a b -> Bool
checkRight' (Left _) = False
checkRight' (Right _) = True

leftValue :: Either a b -> a
leftValue (Left x) = x

rightValue :: Either a b -> b
rightValue (Right x) = x
...
joinToLength n xs = [z | first <-xs, second <- xs, let z = first ++ second, length z == n ]
...
multiApp' :: [(a -> c)] -> a -> [c]
multiApp' [] _ = []
multiApp' (first : rest) x = first x : multiApp' rest x
...
interpreter commands = interpreter' commands 0 0
    where interpreter' ("left" : rest) x y   = interpreter' rest (x - 1) y
          interpreter' ("right" : rest) x y  = interpreter' rest (x + 1) y
          interpreter' ("down" : rest) x y   = interpreter' rest x (y - 1)
          interpreter' ("up" : rest) x y     = interpreter' rest x (y + 1)
          interpreter' ("printX" : rest) x y = show x : interpreter' rest x y
          interpreter' ("printY" : rest) x y = show y : interpreter' rest x y
          interpreter' [] _ _                = []
          interpreter' (_ : rest) x y        = "BAD" : interpreter' rest x y

--interpreter' :: [String] -> Int -> Int -> [String]
--interpreter' [] _ _ = []
--interpreter' (first : rest) x y
--    | first == "left" = interpreter' rest (x - 1) y
--    | first == "right" = interpreter' rest (x + 1) y
--    | first == "up" = interpreter' rest x (y + 1)
--    | first == "down" = interpreter' rest x (y - 1)
--    | first == "printX" = show x : (interpreter' rest x y)
--    | first == "printY" = show y : (interpreter' rest x y)