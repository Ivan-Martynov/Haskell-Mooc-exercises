...
prettyPrint n qs = prettyPrintHelper n (1, 1) (sort qs)
--prettyPrint n cs = prettyPrint' n cs (initString n)

prettyPrintHelper :: Size -> Coord -> [Coord] -> String
prettyPrintHelper n (i, j) ((x, y) : qs)
    | (i, j) == (x, y)  = 'Q' : prettyPrintHelper n (nextCol (i, j)) qs
prettyPrintHelper n (i, j) qs
    | i > n = ""
    | j > n = '\n' : prettyPrintHelper n (nextRow (i, j)) qs
    | otherwise = '.' : prettyPrintHelper n (nextCol (i, j)) qs

initString :: Int -> String
initString n = take (n * (n + 1)) $ cycle $ take n (repeat '.') ++ "\n"

prettyPrint' :: Size -> [Coord] -> String -> String
prettyPrint' n (c : rest) s = prettyPrint' n rest $ replaceToChar (coordToIndex n c) 'Q' s
prettyPrint' _ _ s = s

coordToIndex :: Int -> Coord -> Int
coordToIndex n x = (fst x - 1) * (n + 1) + snd x - 1

replaceToChar :: Int -> Char -> String -> String
replaceToChar n c s = take n s ++ [c] ++ drop (n + 1) s
...
danger c (f : rest)
    | or [sameRow c f, sameCol c f, sameDiag c f, sameAntidiag c f] = True
    | otherwise = danger c rest
danger _ _ = False
...
prettyPrint2 n qs = prettyPrintHelper2 n (1, 1) qs
--prettyPrint2 n qs = prettyPrint2' n (makeAllCoords n) qs (initString n)

prettyPrintHelper2 :: Size -> Coord -> [Coord] -> String
prettyPrintHelper2 n (i, j) qs
    | i > n = ""
    | j > n = '\n' : prettyPrintHelper2 n (nextRow (i, j)) qs
    | (i, j) `elem` qs  = 'Q' : prettyPrintHelper2 n (nextCol (i, j)) qs
    | danger (i, j) qs = '#' : prettyPrintHelper2 n (nextCol (i, j)) qs
    | otherwise = '.' : prettyPrintHelper2 n (nextCol (i, j)) qs

prettyPrint2' :: Size -> Stack -> Stack -> String -> String
prettyPrint2' n (candidate : coords) queens s
    | candidate `elem` queens = prettyPrint2' n coords queens (replaceToChar (coordToIndex n candidate) 'Q' s)
    | danger candidate queens = prettyPrint2' n coords queens (replaceToChar (coordToIndex n candidate) '#' s)
    | otherwise = prettyPrint2' n coords queens s
prettyPrint2' _ [] _ s = s

replicateIndices :: Int -> Int -> [Int]
replicateIndices _ 0 = []
replicateIndices n i = (replicateIndices n (i - 1)) ++ replicate n i

makeAllCoords :: Int -> Stack
makeAllCoords n = zip (replicateIndices n n) (take (n * n) $ cycle [1 .. n])
...
fixFirst n ((x, y) : qs)
    | x > n || y > n = Nothing
    | danger (x, y) qs = if y < n then fixFirst n (nextCol (x, y) : qs) else Nothing
    | otherwise = Just ((x, y) : qs)
...
continue ((x, y) : qs) = nextRow (x, y) : (x, y) : qs
continue _ = [(1, 1)]

backtrack :: Stack -> Stack
backtrack (_ : (a, b) : qs) = nextCol (a, b) : qs
backtrack _ = []
...
step n qs = maybe (backtrack qs) (\y -> continue y) $ fixFirst n qs
