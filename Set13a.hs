...
split s = if (>1) $ length result then Just $ (result !! 0, result !! 1) else Nothing
  where result = words s
...
checkNumber (forename, surname)
  | hasNoNumber forename && hasNoNumber surname = Just (forename, surname)
  | otherwise = Nothing
      where hasNoNumber = null . intersect ['0' .. '9']
...
checkCapitals (forename, surname)
  | startWithCapital forename && startWithCapital surname = Just (forename, surname)
  | otherwise = Nothing
      where startWithCapital s = (head s) `elem` ['A' .. 'Z']
...
winner scores player1 player2 =
  ...
  checkSecond
    where checkSecond x =
            lookup player2 scores ?> \y -> if x < y
                                           then Just player2
                                           else Just player1
...
--selectSum _ [] = Just 0
--selectSum xs (i : is) =
--  safeIndex xs i >>= \x -> selectSum xs is >>= \y -> Just (x + y)
...
countAndLog f (x : xs)
  | f x = msg (show x) >> countAndLog f xs >>= return . (+1)
  | otherwise = countAndLog f xs >>= return
...
paren c = do
  x <- get
  when (x >= 0) (case c of '(' -> put (x + 1)
                           ')' -> put (x - 1)
                           _ -> return ())
--paren c = modify (\x -> if x == -1
--                        then x
--                        else if c == '('
--                             then x + 1
--                             else if c == ')'
...
count x = modify (\y -> count' x y)

count' :: Eq a => a -> [(a, Int)] -> [(a, Int)]
count' x [] = [(x, 1)]
count' x (p : rest)
    | x == fst p = (x, snd p + 1) : rest
    | otherwise = p : count' x rest
