...
countValid xs = length (filter validBanknote xs)
--countValid [] = 0
--countValid (x : xs)
--    | validBanknote x = 1 + countValid xs
--    | otherwise = countValid xs

validBanknote :: String -> Bool
validBanknote s
    | s !! 2 == s !! 4 || s !! 3 == s !! 5 = True
    | otherwise = False
...
sumSuccess (Right x : xs) = Right (foldr (\y acc -> acc + (either (const 0) (id) y)) x xs)
sumSuccess _ = Left "no data"
...
data Lock = LockClosed String | LockOpened String
  deriving Show
...
isOpen :: Lock -> Bool
isOpen (LockOpened _) = True
isOpen _ = False

-- open tries to open the lock with the given code. If the code is
-- wrong, nothing happens.
open :: String -> Lock -> Lock
open code (LockClosed pass) = if code == pass then LockOpened pass else LockClosed pass
open _ l = l

-- lock closes a lock. If the lock is already closed, nothing happens.
lock :: Lock -> Lock
lock (LockOpened pass) = LockClosed pass
lock l = l

-- changeCode changes the code of an open lock. If the lock is closed,
-- nothing happens.
changeCode :: String -> Lock -> Lock
changeCode code (LockOpened pass) = LockOpened code
changeCode _ l = l
...
compose (x : xs) ys = (maybe [] (\y -> [(fst x, y)]) (lookup (snd x) ys)) ++ (compose xs ys)
compose _ _ = []
...
permute perm xs = permuteOnSorted (sortBy (\(_, x) (_, y) -> compare x y) perm) xs

permuteOnSorted :: Permutation -> [a] -> [a]
permuteOnSorted (i : rest) xs = xs !! fst i : permuteOnSorted rest xs
permuteOnSorted _ _ = []