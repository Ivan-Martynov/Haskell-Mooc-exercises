...
instance Eq a => Eq (List a) where
    Empty == Empty = True
    LNode x l1 == LNode y l2 = (x == y) && (l1 == l2)
    _ == _ = False
...
instance Price a => Price [a] where
    price xs = foldr (\x acc -> (price x) + acc) 0 xs
...
instance Eq RationalNumber where
    RationalNumber p q == RationalNumber s t = p * t == q * s
...
simplify (RationalNumber p q) = RationalNumber (p `div` g) (q `div` g) where g = myGcd (abs p) (abs q)
...
instance Num RationalNumber where
    (RationalNumber p1 q1) + (RationalNumber p2 q2) = simplify (RationalNumber (p1 * q2 + p2 * q1) (q1 * q2))
    (RationalNumber p1 q1) * (RationalNumber p2 q2) = simplify (RationalNumber (p1 * p2) (q1 * q2))
    abs (RationalNumber p q) = RationalNumber (abs p) (abs q)
    signum (RationalNumber p q) = RationalNumber (signum p) (signum q)
...
class Cycle a where
    ...
    stepMany :: Int -> a -> a
    stepMany 1 x = step x
    stepMany n x = if n <= 0 then x else stepMany (n - 1) (step x)
