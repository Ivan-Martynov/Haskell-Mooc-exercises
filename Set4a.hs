...
longest :: (Ord a) => [[a]] -> [a]
longest xs = foldr1 (\x acc -> if length x > length acc
                               then x
                               else if length x == length acc
                                    then if x < acc
                                         then x
                                         else acc
                                    else acc) xs
...
incrementKey k xs = foldr (\x acc -> if (fst x == k) then (k, snd x + 1) : acc else x : acc) [] xs
------------------------------------------------------------------------------
...
freqs = foldr freqsHelper Map.empty

freqsHelper :: (Ord a) => a -> Map.Map a Int -> Map.Map a Int
--freqsHelper = \x acc -> Map.alter(\_ -> maybe (Just 1) (\y -> Just (y + 1)) (Map.lookup x acc)) x acc
freqsHelper = Map.alter $ Just . maybe 1 (+1)
...
maxIndex arr = fst $ foldr1 (\x acc -> if snd x > snd acc then x else acc) $ assocs arr