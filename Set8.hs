...
combine f (Picture f1) (Picture f2) = Picture g
...
instance Transform Zoom where
    apply (Zoom n) (Picture f) = Picture (f . (\(Coord x y) -> Coord (x `div` n) (y `div` n)))
...
instance Transform Flip where
    ...
    apply FlipXY (Picture f) = Picture (f . (\(Coord x y) -> Coord y x))
...
instance (Transform a, Transform b) => Transform (Chain a b) where
    apply (Chain f g) p = apply f (apply g p)
...
instance Transform Blur where
    apply Blur (Picture f) = Picture g
        where g (Coord x y) = colorFourNeighbours (f (Coord x y)) (f (Coord (x - 1) y)) (f (Coord (x + 1) y)) (f (Coord x (y - 1))) (f (Coord x (y + 1)))

colorFourNeighbours :: Color -> Color -> Color -> Color -> Color -> Color
colorFourNeighbours (Color r1 g1 b1) (Color r2 g2 b2) (Color r3 g3 b3) (Color r4 g4 b4) (Color r5 g5 b5) =
...
instance Transform BlurMany where
    apply (BlurMany n) picture
        | n <= 0 = picture
        | n == 1 = apply Blur picture
        | otherwise = apply Blur (apply (BlurMany (n - 1)) picture)
