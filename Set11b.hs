...
appendAll s xs = forM_ xs (\x -> modifyIORef s (++x))
...
hSelectLines h nums = hSelectLines' h 1 nums

hSelectLines' :: Handle -> Int -> [Int] -> IO [String]
hSelectLines' h i nums = do
                         b <- hIsEOF h
                         if b then return []
                              else do
                                  s <- hGetLine h
                                  xs <- hSelectLines' h (i + 1) nums
                                  return $ if i `elem` nums then s : xs else xs
...
interact' f state = do
        line <- getLine
        let (b, s, t) = f (line, state)
        putStrLn s
        if b then do interact' f t else return t