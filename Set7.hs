...
data State = Start | Error | Finished | EggsAdded | SugarAdded | FlourAdded | ReadyToMix | ReadyToBake
  deriving (Eq,Show)
...
step ReadyToMix Mix = ReadyToBake
step ReadyToBake Bake = Finished
...
instance Semigroup Distance where
    Distance s1 <> Distance s2 = Distance (s1 + s2)

instance Semigroup Time where
    Time s1 <> Time s2 = Time (s1 + s2)

instance Semigroup Velocity where
    Velocity s1 <> Velocity s2 = Velocity (s1 + s2)
...
--passwordAllowed p (DoesNotContain s) = null $ intersect p s
--passwordAllowed p (ContainsSome s) = not $ passwordAllowed p (DoesNotContain s)
passwordAllowed p (ContainsSome s) = any (\c -> c `elem` s) p
