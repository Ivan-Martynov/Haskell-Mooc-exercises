...
mapM2 op xs = mapM id . zipWith op xs
...
path maze place1 place2 = place2 `elem` paths
  where (_, paths) = runState (visit maze place1) []
...
allSums :: [Int] -> [Int]
allSums xs = map (sum) $ filterM (\x -> [True, False]) xs
...
instance Monad SL where
  ...
  return x = SL (\i -> (x, i, []))
  op >>= f = SL h
    where h state0 = let (val1, state1, xs1) = runSL op state0
                         op2 = f val1
                         (val2, state2, xs2) = runSL op2 state1
                     in (val2, state2, xs1 ++ xs2)
...
mkCounter =
  do
    c <- newIORef 0
    return (f c, g c)
      where f y = modifyIORef y (+1)
            g y = readIORef y >>= return