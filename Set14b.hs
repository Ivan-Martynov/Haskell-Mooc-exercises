...
balance db account =
  do
    sums <- query db balanceQuery [account]
    return $ sum $ concat sums
...
parseCommand [command, account] =
  if command == T.pack "balance"
  then Just (Balance account)
  else Nothing
parseCommand [command, account, amount] =
  if command == T.pack "deposit"
  then case  parseInt amount of
    Just x -> Just (Deposit account x)
    Nothing -> Nothing
  else if command == T.pack "withdraw"
       then case  parseInt amount of
         Just x -> Just (Withdraw account x)
         Nothing -> Nothing
       else Nothing
...
perform db (Just (Withdraw account amount)) =
  do
    ...
    return (T.pack "OK")
perform db (Just (Balance account)) =
  do
    sum <- balance db account
    return (T.pack $ show sum)
perform db _ = return (T.pack "ERROR")
