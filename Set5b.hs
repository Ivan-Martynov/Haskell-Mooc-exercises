...
treeMax Empty = 0
treeMax (Node x l r) = max (max x $ treeMax l) $ treeMax r
...
search val t = roadTo val (road t)

roadTo :: Eq a => a -> [[(a, Step)]] -> Maybe [Step]
roadTo val paths = let path = (foldr (const) []) $ filter (\x -> elem val (map (fst) x)) paths
    in if path == [] then Nothing else Just (map (snd) (takeWhile (((/=) val) . fst) path))

road :: Eq a => Tree a -> [[(a, Step)]]
road Empty = []
road (Node x l r)
    | road l ++ road r == [] = [[(x, StepL)]]
    | otherwise = map (((x, StepL):)) (road l) ++ map (((x, StepR):)) (road r)

searchV2 :: Eq a => a -> Tree a -> Maybe [Step]
searchV2 _ Empty = Nothing
searchV2 val (Node x l r)
    | val == x = Just []
    | otherwise = case searchV2 val l of
                    Just xs -> Just (StepL : xs)
                    Nothing -> case searchV2 val r of
